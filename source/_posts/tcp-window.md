...
- 第三次`ACK`: 我们告知服务器，由于双方都有相通的窗口缩放因子256，因此我们的接收窗口根据当前处理能力修改为87808Bytes(`343*256`)

之后服务端根据我们的接收窗口大小，以及服务端的拥塞窗口大小决定其发送窗口大小，并根据协定的`MSS`一次发送多个报文段给本地TCP的接收窗口缓存，后面我们本地的TCP根据处理能力不断调整接收窗口大小，也就是说这块虽然我们应用层没有主动的不断读取输入流，但是TCP的接收窗口已经缓存了数据。

### II. 基本概念

#### 什么是MTU(Maximum Transmit Unit)

由于以太网传输的限制，每个以太网帧都是在区间`[64Bytes,1518Bytes]`，不在区间内的一般会被视为错误的数据帧，以太网转发设备直接丢弃。而根据以太网每帧的数据构成，除去固定的部分，留给上层协议的只有`Data`域的1500Bytes，我们将它称为MTU。
