...
将小蚁摄像头的内存卡取出来，在内存卡根目录创建一个`test`的文件夹，注意如果是你的内存卡被分区为两个，在较大的那个里面进行创建，并在该`test`文件夹下面创建文件`equip_test.sh`粘贴以下内容:
...
现在将内存卡插回小蚁摄像头，然后将摄像头开机，此时不出意外，你应该就可以直接通过`telnet 192.168.99.111`(假设你的小蚁摄像头内网IP是`192.168.99.111`)访问，输入用户名`root`，以及密码`123qwer`就可以访问了
...
然后我们将小蚁摄像头的内存卡拿出来，将刚刚打包好的`target.tgz`拷贝到小蚁摄像头内存卡的较大的那个分区的根目录，然后插回小蚁摄像头，然后启动小蚁摄像头。
...
由于内存卡是`fat32`的格式，不支持软连接，因此这边需要对关键的几个`so`进行手动的处理:
